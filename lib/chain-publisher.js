...
class ChainPublisher {
    ...
    async _processLogs({ key, logs, subcriber, memories, requests, head }) {
        ...
        await subcriber.applyLogs(
            {
                setItem: (key, value) => memories.set(key, value),
                getItem: key => this.config.storage.getItem(key),
                removeItem: key => memories.set(key, REMOVE_KEY)
            },
            unsafeLogs,
            true
        )
