...
class ChainPublisher {
    ...
    async _processor(head) {
        ...
        for (const [key, subcriber] of this.subscribers) {
            const request = requestsByKey[key]
            if (!request) {
                return
            }

            const consumerLogs = filterLogs(logs, request)

            await this._processLogs({ key, logs: consumerLogs, subcriber, request, head })
        }

        return head
    }

    async _processLogs({ key, logs, subcriber, request, head }) {
        let safeBlock = _.max([request.from - 1, head - subcriber.safeDepth])
        ...
        console.log(`\t --- SAFE   ${request.from} +${safeBlock - request.from + 1} :${safeLogs.length}\t${key}`)
