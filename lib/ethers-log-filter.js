...
exports.equalLog = (log, logCompare) => {
    return log.transactionHash == logCompare.transactionHash && log.logIndex == logCompare.logIndex
}

function _compareLog(a, b) {
...
exports.mergeTwoUniqSortedLogs = (a, b) => {
    if (!a?.length) {
        return b ?? []
    }
    if (!b?.length) {
    ...
    while (i.a < a.length || i.b < b.length) {
        ...
        const c = _compareLog(a[i.a], b[i.b])
        if (c < 0) {
            r.push(a[i.a++])
            continue
        }
